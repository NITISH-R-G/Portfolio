...
/**
 * A preview route, under the deployment's base path.
 *
 * `basePath` is applied by Next to `<Link>` and the router, but not to a string handed to an
 * iframe `src` — so these URLs have to carry it themselves or they 404 on a project site.
 * Verified: `/preview/hero-01/` is a 404 under `/Portfolio`, `/Portfolio/preview/hero-01/` is 200.
 */
const previewPath = (name: string) =>
  `${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/preview/${name}`
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <div className="flex h-8 items-center gap-0.75 rounded-lg border p-0.75">
          ...
          <Button
            ...
            render={
              <a
                href={serializePreviewSearchParams(previewPath(item.name), {
                  theme,
                })}
                target="_blank"
                rel="noopener"
...
function BlockViewerIframe({ className }: { className?: string }) {
  ...
  const iframeSrc = useMemo(
    () => {
      ...
      return serializePreviewSearchParams(previewPath(item.name), { theme })
    },
